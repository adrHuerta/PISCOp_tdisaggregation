...
# C2,3
C_persiann_gsmap = sum((r_persiann - mean(r_persiann))*(r_gsmap - mean(r_gsmap)))/8760
# C2,2
C_persiann_persiann = sum((r_persiann - mean(r_persiann))*(r_persiann - mean(r_persiann)))/8760
# C3,3
C_gsmap_gsmap = sum((r_gsmap - mean(r_gsmap))*(r_gsmap - mean(r_gsmap)))/8760

# var1
var_imerg = C_imerg_imerg - (C_imerg_persiann*C_imerg_gsmap)/C_persiann_gsmap
var_persiann = C_persiann_persiann - (C_imerg_persiann*C_persiann_gsmap)/C_imerg_gsmap
var_gsmap = C_gsmap_gsmap - (C_imerg_gsmap*C_persiann_gsmap)/C_imerg_persiann

#var_imerg_r = var_imerg*mean(imerg_early_files)
#var_persiann_r = var_persiann*mean(persiann_css_files)
#var_gsmap_r = var_gsmap*mean(gsmap_op_files)

#VAR_imerg_2 = ((1/mean(imerg_early_files))^2)*(var_imerg_r^2)
VAR_imerg_2 = var_imerg*var_imerg
#VAR_persiann_2 = ((1/mean(persiann_css_files))^2)*(var_persiann_r^2)
VAR_persiann_2 = var_persiann*var_persiann
#VAR_gsmap_2 = ((1/mean(gsmap_op_files))^2)*(var_gsmap_r^2)
