...
xr.apply_ufunc(get_time_of_max_value, cycle_grid, input_core_dims=[['time2']], vectorize=True).to_netcdf("paper/output/PISCOp_hourly_hourly_cycle_maxvalue_" + "_".join(seasons[season]) + ".nc")
...
#
files_grid_0 = sorted(glob.glob("data/processed/gridded/PISCOp_hourly_mean_SAT/*-" + seasons[season][0] + "-*.nc"))
files_grid_1 = sorted(glob.glob("data/processed/gridded/PISCOp_hourly_mean_SAT/*-" + seasons[season][1] + "-*.nc"))
files_grid_2 = sorted(glob.glob("data/processed/gridded/PISCOp_hourly_mean_SAT/*-" + seasons[season][2] + "-*.nc"))
...
cycle_grid = xr.apply_ufunc(hourly_cycle, merged_files_grid, input_core_dims=[['time']], output_core_dims=[['time2']],
                            output_sizes={"time2": 24}, output_dtypes="float32", vectorize=True, dask="parallelized")
cycle_grid.to_netcdf("paper/output/PISCOp_hourlymeanSAT_hourly_cycle_" + "_".join(seasons[season]) + ".nc")
cycle_grid = xr.open_dataset("paper/output/PISCOp_hourlymeanSAT_hourly_cycle_" + "_".join(seasons[season]) + ".nc")
xr.apply_ufunc(get_time_of_max_value, cycle_grid, input_core_dims=[['time2']], vectorize=True).to_netcdf("paper/output/PISCOp_hourlymeanSAT_hourly_cycle_maxvalue_" + "_".join(seasons[season]) + ".nc")
xr.apply_ufunc(amplitude_cycle, cycle_grid, input_core_dims=[['time2']], vectorize=True).to_netcdf("paper/output/PISCOp_hourlymeanSAT_hourly_cycle_amplitude_" + "_".join(seasons[season]) + ".nc")
...
xr.apply_ufunc(get_time_of_max_value, cycle_grid, input_core_dims=[['time2']], vectorize=True).to_netcdf("paper/output/AWSs_hourly_hourly_cycle_maxvalue_" + "_".join(seasons[season]) + ".nc")
xr.apply_ufunc(amplitude_cycle, cycle_grid, input_core_dims=[['time2']], vectorize=True).to_netcdf("paper/output/AWSs_hourly_hourly_cycle_amplitude_" + "_".join(seasons[season]) + ".nc")
