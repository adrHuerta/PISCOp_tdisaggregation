...
for season in range(len(seasons)):
  ...
  merged_files_grid = xr.open_dataset(merged_files_grid, chunks={"latitude": 100, "longitude": 100})
  intensity_grid = xr.apply_ufunc(mean_no_zero, merged_files_grid, input_core_dims=[['time']],
                                  output_dtypes=['float32'], vectorize=True, dask="parallelized")
  intensity_grid.to_netcdf("paper/output/PISCOp_hourlymeanSAT_intensity_" + "_".join(seasons[season]) + ".nc")
...
for season in range(len(seasons)):
  ...
  intensity_grid = xr.apply_ufunc(mean_no_zero, merged_files_grid, input_core_dims=[['time']],
                                  output_dtypes=['float32'], vectorize=True, dask="parallelized")
  intensity_grid.to_netcdf("paper/output/AWSs_hourly_intensity_" + "_".join(seasons[season]) + ".nc")
