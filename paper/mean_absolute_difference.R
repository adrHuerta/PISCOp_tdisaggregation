...
lapply(seasons, function(z){
  ...
  rbind(data.frame(value = MAD_1, as.data.frame(aws_sp_vi)[, c("LAT", "LON")], data = rep("PISCOp_hourly/AWS", length(MAD_1)), season = rep(paste(z, collapse = "_"), length(MAD_1))),
        data.frame(value = MAD_2, as.data.frame(aws_sp_vi)[, c("LAT", "LON")], data = rep("PISCOp_hourly_noBC/AWS", length(MAD_2)), season = rep(paste(z, collapse = "_"), length(MAD_2)))
  )
...
ggsave(file.path(".", "paper", "output", "Fig_MAD.jpg"),
       dpi = 300, scale = 1,
