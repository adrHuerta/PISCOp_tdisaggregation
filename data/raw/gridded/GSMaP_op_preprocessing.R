...
files_early = sorted(glob.glob("./data/raw/gridded/GSMaP_op/*.nc"))
...
for i_year in files_early:
  ...
  if i_year_str == "2014":
    i_time_range = pd.date_range(start="2014-03-01 00:00:00", end="2014-12-31 23:00:00", freq="h")
  else:
  ...
  gpmimerg_early.to_netcdf("./data/processed/gridded/GSMaP_op/" + i_year_str + "_gsmap_op_p.nc", encoding=encoding,
                           engine='netcdf4')


#·····································#
  
path_netcdf_in = "./data/processed/gridded/GSMaP_op/"
path_netcdf_out = "./data/processed/gridded/GSMaP_op/"
  
for year in range(2014,2021):
  file_year = xr.open_dataset(path_netcdf_in + str(year) + "_gsmap_op_p.nc")
  ...
  np.round(file_merged, 1).to_netcdf(path_netcdf_out + str(year) + "_gsmap_op.nc", encoding=encoding, engine='netcdf4')
